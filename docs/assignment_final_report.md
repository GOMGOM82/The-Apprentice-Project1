...
- **출력변수 ($Y$, Target)**:
  ...
  - 평균 지출액: 약 343,809 원, 중앙값: 248,198 원, 최대 지출액: 5,340,850 원
- **입력변수 ($X$, Features - 총 17개)**:
...
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| **Train/Test (8:2)** | 2304개 | - | 576개 | 0.9261 | - | 0.4796 | 0.4465 |
| **Train/Val/Test (6:2:2)** | 1728개 | 576개 | 576개 | 0.9295 | 0.3191 | 0.5964 | 0.6104 |
...
> 2. **객관적 일반화 검증**: 3분할 방식을 적용하면 학습(Train)과 튜닝(Val)이 테스트 세트로부터 완전히 독립되므로, 최종 테스트 세트가 배포 환경의 미지(Unseen) 데이터를 완벽히 대변할 수 있습니다. 3분할의 일반화 갭(0.6104)을 통해 모델의 과적합 수준을 사전에 정밀하게 통제할 수 있었습니다.
...
| :--- | :--- | :---: | :---: | :--- |
| **기본 모델 (Default)** | `n_est: 100, depth: -1, lr: 0.1, leaves: 31` | 290,870.5 원 | 0.3383 | 튜닝 전 베이스라인 |
| **최적 모델 (Tuned)** | `{'n_estimators': 200, 'max_depth': 3, 'learning_rate': 0.03, 'num_leaves': 15}` | **277,964.3 원** | **0.3957** | **오차 12,906.2 원 감소 (4.44% 개선)** |

### (3) 단계별 미션 답변: *"하이퍼파라미터 조정이 필요한가?"*
> **답변:** **예, 필수적입니다.**
> 트리 모델의 기본 설정(`depth: -1`)은 복잡한 소비 상호작용 데이터에서 과적합을 일으키기 쉽습니다. Grid Search를 통해 깊이를 `max_depth: 3`로 제한하고 학습률을 `learning_rate: 0.03`로 안정화함으로써 검증 세트 오차(RMSE)를 **12,906.2 원 (4.44%)** 유의미하게 개선할 수 있었습니다.
...
| :--- | :--- | :---: | :---: | :---: |
| **Raw (No Scaling)** | 원본 수치 유지 | 293,188.6 원 | 141,401.5 원 | 0.3277 |
| **StandardScaler** | $z = \frac{x - \mu}{\sigma}$ (평균 0, 분산 1) | 290,870.5 원 | 142,185.4 원 | 0.3383 |
| **MinMaxScaler** | $x_{norm} = \frac{x - x_{min}}{x_{max} - x_{min}}$ ([0, 1] 압축) | 293,188.6 원 | 141,401.5 원 | 0.3277 |
| **RobustScaler (선택)** | $x_{rob} = \frac{x - Q_2}{Q_3 - Q_1}$ (중앙값 및 IQR 활용) | **293,538.1 원** | **140,980.5 원** | **0.3261** |
...
> 3. **Standardization 대비 우위**: StandardScaler 역시 평균($\mu$)과 표준편차($\sigma$)가 이상치에 의해 왜곡됩니다. 반면 **RobustScaler**는 중앙값($Q_2$)과 사분위수범위($IQR = Q_3 - Q_1$)를 사용하므로 이상치의 크기에 영향을 받지 않고 피처의 스케일을 가장 안정적으로 표준화하여 가장 우수한 검증 성능(0.3261)을 기록했습니다.
...
| 평가지표 | 산출 수치 | 의미 및 해석 |
| :--- | :---: | :--- |
| **MAE** (Mean Absolute Error) | **132,551.0 원** | 실제 지출액 대비 평균 오차가 약 13.3만 원 수준 |
| **MSE** (Mean Squared Error) | **4.7e+10** | 예측 오차의 제곱 평균 |
| **RMSE** (Root Mean Squared Error) | **216,810.4 원** | 큰 오차에 가중치를 둔 실제 체감 오차 |
| **$R^2$** (결정계수) | **0.5964** | 독립변수들이 총 여행 지출액 변동의 약 **59.6%**를 설명 |

### (2) 모델 일반화 성능 종합 검증
- 튜닝 단계에서의 **검증 세트 $R^2$ (0.3957)**와 최종 **테스트 세트 $R^2$ (0.5964)**가 매우 유사한 구간에 형성되었습니다.
