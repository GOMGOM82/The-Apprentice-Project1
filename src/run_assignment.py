...
### (3) 원천 데이터(89.6 GB) 중 특정 일부 데이터만 추출·활용한 구체적 사유
AI-Hub 원천 데이터(#71778)는 사진 74GB, GPS 트랙 207MB, POI 마스터 1.7GB, 가공 라벨 14.5GB 및 관계형 테이블을 포함한 총 **89.6 GB의 방대한 멀티모달 데이터셋**입니다. 본 프로젝트에서 **사전 성향/프로필 및 소비 지출 결합 데이터(2,880건, 14개 특징 변수)**를 선별 추출한 학술적·공학적 근거는 다음과 같습니다:

1. **도메인 연구 목적과의 정합성 (사전 성향 기반 지출액 추정)**:
   - 본 과제의 목적은 *"여행을 떠나기 전 수집 가능한 여행객의 라이프스타일 성향(8대 스타일)과 인구통계학적 프로필만으로 총 소비 지출액을 사전에 추정하는 것"*입니다.
   - 원천 데이터의 80% 이상을 차지하는 고용량 사진 파일(74GB)과 초 단위 GPS 트래킹 좌표(207MB)는 여행 도중/종료 후에 발생하는 산출물이므로 '여행 전 성향 모델링' 목적에 부합하지 않아 정밀 제외했습니다.

2. **타깃 누락(Target / Data Leakage)의 원천 차단**:
   - 원천 방문지 테이블(`tn_visit_area_info_`)의 실제 방문 장소 수(`VISIT_AREA_NUM`), 방문지 체류 시간(`STAY_TIME_MIN`), 사후 만족도(`DGSTFN`), 추천 의향 등은 **소비 지출이 이미 완료된 후에만 관측 가능한 사후 결과 변수**입니다.
   - 이러한 변수를 학습에 포함할 경우 모델이 사후 결과를 보고 소비액을 맞추는 인과관계 역전(Data Leakage)이 발생하므로, 실전 사전 예측 가치를 보장하기 위해 엄격히 배제했습니다.

3. **데이터 계층 구조 정규화 및 분석 단위(Trip Level) 일치**:
   - 원천 데이터는 GPS 포인트(수백만 행), 방문지(수만 행), 지출 영수증(수만 행) 등 서로 다른 계층(Granularity)으로 분산되어 있습니다.
   - 분석 단위인 **'여행자 1인의 1회 여행 총 지출액'**에 일치시키기 위해, 여행 마스터(`tn_travel_`) 및 여행자 프로필(`tn_traveller_master_`)을 기준으로 사전 예약금(`ADV_CONSUME_KRW`)과 현장 지출(`tn_activity_consume_`)을 여행 ID(`TRAVEL_ID`) 단위로 완벽히 집계·정규화하여 2,880건의 고품질 분석 매트릭스를 구축했습니다.

4. **연산 자원 최적화 및 신호 대 잡음비(Signal-to-Noise Ratio) 극대화**:
   - 89.6 GB 전체 데이터를 직접 메모리에 적재할 경우 불필요한 이미지 픽셀 및 고주파 GPS 노이즈로 인해 극심한 I/O 병목이 발생합니다.
   - 소비 지출과 실질적 인과/상관관계를 가지는 정형(Tabular) 사전 변수 14개만 선별 추출하여 모델 학습 시간을 수 초 단위로 단축하고 노이즈를 완벽히 통제했습니다.
